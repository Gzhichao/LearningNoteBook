...
do
    ...
    LotteryNum=$(grep -ar '"isEligible": true, "isWinner": false' $WinningLog |grep "${DateTime}T" -c)
    WinNum=$(grep -ar '"isEligible": true, "isWinner": true'  $WinningLog |grep "${DateTime}T" -c)
    MinedNewBlockNum=$(grep -c "${DateTime}T" $MinedNewBlock)
